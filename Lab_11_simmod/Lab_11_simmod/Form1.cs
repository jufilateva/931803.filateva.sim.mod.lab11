...
namespace Lab_9_simmod
{
    public partial class Form1 : Form
    {
        ...
        {
            double Aemp = 0, A = 0, Vemp = 0, V = 0;
            double deltaA; double deltaV;
            ...
            V -= A * A; Vemp -= Aemp * Aemp;

            deltaA = Math.Abs(A - Aemp) / Math.Abs(A);
            deltaV = Math.Abs(V - Vemp) / Math.Abs(V);
            av.Text = Aemp.ToString("F5");
            var.Text = Vemp.ToString("F5");
            av_error.Text = deltaA.ToString("F5");
            var_error.Text = deltaV.ToString("F5");
            ...
            {
                chi.Text = "TRUE";
                chi.ForeColor = Color.Green;
            }
            else
            {
                chi.Text = "FALSE";
                chi.ForeColor = Color.Red;
            }
        }
